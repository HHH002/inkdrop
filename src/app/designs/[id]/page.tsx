...
export default function DesignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-dvh bg-white pb-36">
        ...
        >
          ...
          {carouselIndex > 0 && (
            <button
              ...
              className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white backdrop-blur rounded-full flex items-center justify-center shadow-sm active:scale-95 transition-transform"
            >
          ...
          {carouselIndex < CAROUSEL_PATTERNS.length - 1 && (
            <button
              onClick={() => setCarouselIndex(i => i + 1)}
              className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white backdrop-blur rounded-full flex items-center justify-center shadow-sm active:scale-95 transition-transform"
            >
      ...
      {/* 固定購入バー */}
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] bg-white backdrop-blur border-t border-gray-100 px-4 py-4 z-40">
